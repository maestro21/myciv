<?php class modules extends masterclass {
	...
	function dbupdate() {
        $this->ajax = true;
        $dblastupdate = G('db_lastupdate',1);
        include(BASE_PATH . 'data/db/dbupdates.php');
        $dblastupdate = 0;
        if($dblastupdate <= $lastupdate) {
            for($i = $dblastupdate; $i <= $lastupdate; $i++) {
                $do = 'update_' . $i; echo $do;
                if(function_exists($do)) {
                    dbupdate($do());
                }
            }
            G('db_lastupdate', $lastupdate);
            return json_encode(array( 'status' => 'ok', 'msg' => T('dbupdated')));
        }
        return json_encode(array( 'status' => 'info', 'msg' => T('up to date')));
    }
