	...
	foreach($tables as $table_name => $table) {
		...
		foreach ($table['fields'] as  $field_name => $field){
			...
			switch($type){
                ...
                case DB_CHAR: $type = 'VARCHAR(1)'; break;
				default : $type = '';	
			}
...
/**
 * @param $update - array with following format:
 * [
 *  'modulename' => [
 *      'fieldname' => [
 *          'type'  => %TYPENAME%, - standard type, or index|unique for dropping index
 *          'do'    => %ACTIONNAME% - can be CHANGE, DROP or empty. If left empty, will try to add column.
 *  ],
 * ]
 * Some examples:
 *  'testfield' => [ 'type' => 'text' ] - will output ADD COLUMN `testfield` text
 *  'testkey' => [ 'type' => 'unique' ] - will output ADD UNIQUE(testkey)
 *  'testfield' => ['newname' => 'testfield2', 'do' => 'change' ] - will outtup CHANGE `testfield` testfield2
 *  'testkey' => [ 'do' => 'DROP' ]  - will output DROP INDEX(testkey)
 */
function dbupdate($update) {// print_r($update);
