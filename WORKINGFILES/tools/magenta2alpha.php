<?php
$directory = $input_dir;
$dir = array_diff(scandir($directory), array('..', '.')); natsort($dir);

foreach($dir as $filename) { 
	$fpath = $directory . '\\' . $filename;		
	if(is_dir($fpath)) continue;
	$size = getimagesize($fpath); //print_r($fpath); print_r($size);

	$x = $size[0];
	$y = $size[1];
	$im = imagecreatetruecolor($x, $y);
	imagealphablending($im, false);
	imagesavealpha($im, true);
	$alpha = imagecolorallocatealpha($im, 0, 0, 0, 127);
	imagefill($im, 0, 0, $alpha );		
	
	switch($size['mime']) {
		case 'image/png': $img = imagecreatefrompng($fpath); break;
		case 'image/gif': $img = imagecreatefromgif($fpath); break;
		case 'image/jpeg': $img = imagecreatefromjpeg($fpath); break;
	}
	if(isset($img)) {
		for($i = 0 ; $i < $x; $i++) {
			for($j = 0; $j < $y; $j++) {
				$rgba = imagecolorsforindex($img,imagecolorat($img, $i, $j));
				$r = $rgba['red'];//($rgb >> 16) & 0xFF;
				$g = $rgba['green'];//($rgb >> 8) & 0xFF;
				$b = $rgba['blue'];//$rgb & 0xFF;
				$alpha = $rgba['alpha'];
				if($r == $b && $r != $g) {
					$alpha = round(($r + $b) / (255 * 2) * 127);
					//echo $alpha;
					$color = imagecolorallocatealpha($im, 0,0,0,$alpha); 
				} else {
					$color = imagecolorallocate($im, $r,$g,$b); 
				}
				imagesetpixel($im,$i,$j,$color); 				
			}		
		}
		imagepng($im, $output_dir . '/' . $filename);
		imagedestroy($im);
	}
}
echo "Conversion from magenta to alpha done";



/*
	for($i = 0 ; $i < $x; $i++) {
		for($j = 0; $j < $y; $j++) {
			$rgb = imagecolorat($img, $i, $j);
			$index = imagecolorsforindex($img, $rgb);
			//print_r($index);
			$r = $index['red'];
			$g = $index['green'];
			$b = $index['blue'];
			if($r == $b && $r != $g) { 
				$alpha = floor($r / 2); 
				$color = imagecolorallocatealpha($im, 0,0,0,$alpha); 
			} else { 
				$color = imagecolorallocatealpha($im, $r,$g,$b,0); 
			}
			imagesetpixel($im, $ox + $i, $oy + $j, $color); 				
		}		
	}*/