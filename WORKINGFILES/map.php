...
class Map {
	...
	function __construct() {
		...
		$this->terrain = conf('terrain');
		$this->screen = conf('screen');
			...
			{	
				...
				for($x =  - 1; $x <= $this->screen['tilesX']; $x++) {
					...
					if(isset($this->terrain2[$_terrain])) {
						$tile['terrain2'] = $_terrain;
						$_terrain = $this->terrain2[$_terrain];
					} 
					$tile['terrain'] = $_terrain;
					/** adding coast **
					if($_terrain == 'water') {
						$this->getCoast($dx, $dy);
					}
