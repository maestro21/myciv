...
class Map {
	...
	function loadMap($map = 'demomap', $offsetx = 0, $offsety = 0) { //die('11');		
		...
		for($j = -1; $j < $y + 1; $j++) {
			...
			for($i = -1 ; $i < $x + 1; $i++) {
				$tilex = $i + $offsetx;		
				if($tilex < 0) $tilex = $sizex + $tilex;
				if($tilex > $sizex) $tilex = $tilex - $sizex;	
				if($tiley < 0 || $tiley > $sizey) {
					$_tile = 'snow';
				} else {
					$rgb = imagecolorat($img, $tilex, $tiley); 
			...
			if(file_exists($fpath)) {
				...
				for($j = 0; $j < $y; $j++) {
					...
					for($i = 0 ; $i < $x; $i++) {
						...
						if($tilex < 0) $tilex = $sizex + $tilex;
						if($tilex >= $sizex) $tilex = $tilex - $sizex;	
						if($tiley < 0 || $tiley > $sizey) continue;
